...
using namespace std;
void MeasurementEquationFunction_test :: ConvertXtoParameter(const TDenseVector &x)
{
	scl4Sigmai2 = fixed_parameter(0);
        ...
        Phiy = x(i_);                   i_++;
        RhoMu1 = x(i_);         RhoMu2 = x(j_);         i_++;   j_++;   // Markup
        Rhorn1 = x(i_);         Rhorn2 = x(j_);         i_++;   j_++;   // Natural rate of interest
        Rhoi1 = x(i_);          Rhoi2 = x(j_);          i_++;   j_++;   // Monetary policy
        SigmaMu1 = x(i_);       SigmaMu2 = x(j_);       i_++;   j_++;   // Markup
        Sigmarn1 = x(i_);       Sigmarn2 = x(j_);       i_++;   j_++;   // Natural rate of interest
        Sigmai1 = x(i_);        Sigmai2 = x(j_)/scl4Sigmai2;    i_++;   j_++;   // Monetary policy, 1) 1 (or 2.5) basis point annually = 2.5000e-05 (or 6.2500e-05) quarterly.  2) 2.5000e-04 quarterly = 10 basis points annually in the ZLB.
