...
CEquiEnergy_CMSSM_test :: CEquiEnergy_CMSSM_test(bool _if_bounded, int eL, double _t, const CSampleIDWeight &_x, time_t _time, CMetropolis *_metropolis, CEESParameter *_parameter, CStorageHead *_storage, CMSSM_test *_model) : CEquiEnergyModel(_if_bounded, eL, _t, _x, _time, _metropolis, _parameter, _storage), target_model(_model), y(vector<TDenseVector>(0)), z_0(vector<TDenseVector>(0)), P_0(vector<TDenseMatrix>(0)), initial_prob(TDenseVector(0))
{}

CEquiEnergy_CMSSM_test :: ~CEquiEnergy_CMSSM_test()
{}

void CEquiEnergy_CMSSM_test::SaveSampleToStorage(const CSampleIDWeight &sample)
{
