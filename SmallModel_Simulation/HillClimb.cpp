...
int HillClimb(double &log_posterior_optimal, TDenseVector &x_optimal, const TDenseVector &x0, CMSSM_test_1st &model_1st, CMSSM_test_2nd &model_2nd, CMSSM_test_1st & model_all, const vector<int> &locs_x1, const vector<int> &locs_x2, const vector<int> &locs_xall, const vector<TDenseVector> &y1st, const vector<TDenseVector> &y2nd, const vector<TDenseVector> &y)
{
	...
	log_posterior_optimal = CMSSM::MINUS_INFINITY_LOCAL; 
	...
	for (unsigned int i_try=0; i_try<n_try; i_try++)
	{
		...
		if (!bad)
		{
			if (log_posterior_all > log_posterior_optimal)
			{
				log_posterior_optimal = log_posterior_all; 
				x_optimal.CopyContent(x_input); 
			}
		}		
	}
