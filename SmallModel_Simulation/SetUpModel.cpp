...
void SetUpModel(size_t nY, TDenseVector &fixed_parameter, vector<int> &locs_x1, vector<int> &locs_x2, vector<int> &locs_xall, CMSSM_test_1st &model_1st, CMSSM_test_2nd &model_2nd, CMSSM_test_1st &model_all) 
{
	...
	model_1st=CMSSM_test_1st(locs_x1, nL, nTL, nS, nZ, nY, nU, nE, nExpectation, rational_expectation_func, state_equation_func, measurement_equation_func, transition_prob_func, new PriorDistrFunction_test_1st(fixed_parameter)); 

	model_2nd=CMSSM_test_2nd(locs_x2, nL, nTL, nS, nZ, nY, nU, nE, nExpectation, rational_expectation_func, state_equation_func, measurement_equation_func, transition_prob_func, new PriorDistrFunction_test_2nd(fixed_parameter) ); 
	
	model_all=CMSSM_test_1st(locs_xall, nL, nTL, nS, nZ, nY, nU, nE, nExpectation, rational_expectation_func, state_equation_func, measurement_equation_func, transition_prob_func, new PriorDistrFunction_test_all(fixed_parameter) );
